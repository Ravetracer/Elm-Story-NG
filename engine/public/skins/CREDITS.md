...
paperdoll, choice modal and panels when a storyworld selects a skin. The money
display in the object rail — the currency coin and the plate it sits on — is from
the same source and is part of the base chrome: it shows in every storyworld, skin
or none, embedded in the engine stylesheet. All of it is derived from the Wenrexa
GUI asset kits and is used and redistributed under the **Creative Commons
...
| money plate (base chrome) | Wenrexa "GUI Game #6 [RPG, RTS, MMO, Action, Medieval]" | `PanelSettings_Slider.png` | re-encoded to WebP and embedded as a data URI in the engine stylesheet; metadata stripped, no pixels redrawn |
